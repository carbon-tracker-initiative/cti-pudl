...
COLS_CAPEX = ["capex_total_shifted", "capex_annual_addt", "capex_annual_addt_rolling"]
...
PATH_GEM = PATH_INPUTS / "gem_gas_units_updated.xlsx"
...
def make_ct_compilation(pudl_out, unit_id_col):
    ...
    gens_w_ferc1 = (
        merge_eia_ferc(gen=gen, steam_df=steam_df, unit_id_col=unit_id_col, unit=unit)
...
def add_state_fuel_avg(out_df):
    ...
    state_fuel_avg = weighted_average(
        out_df, {"fuel_cost_per_mmbtu": "net_generation_mwh"}, idx_fuel_state
    )
    out_df_w_avg = out_df.merge(
        state_fuel_avg,
        how="left",
        on=idx_fuel_state,
        validate="m:1",
        ...
        fuel_cost_source=lambda x: np.where(
            x.fuel_cost_per_mmbtu.notnull(),
            "plant-level",
            np.where(x.fuel_cost_per_mmbtu_state_avg.notnull(), "state-level", pd.NA),
        ),
        fuel_cost_per_mmbtu=lambda x: x.fuel_cost_per_mmbtu.fillna(
            x.fuel_cost_per_mmbtu_state_avg
        ),
    )
    return out_df_w_avg
...
def prep_gens_eia(pudl_out):
    ...
    # from mcoe
    drop_cols = [
        x for x in pudl_out.gens_eia860() if x in pudl_out.mcoe() and x not in IDX_GEN
    ]
    ...
    gen = (
        ...
        # add the ferc_acct
        .merge(
            pd.read_csv(pathlib.Path().cwd().parent / "inputs/ferc_acct_map.csv")[
...
def agg_gen_eia_to_unit(gen, unit_id_col):
    ...
    """
    logging.info("Aggergating the generators to a unit level. This takes some minutes.")
    idx_unit = ["plant_id_eia", "report_date", unit_id_col]
    # prep the categorical data aggregation
    # we want to prioritize some
    gen = gen.astype(
        {
            "operational_status": pd.CategoricalDtype(),
        }
    )
    ...
    gen_gpb = (
        gen.groupby(idx_unit, dropna=False).agg(agg_dict, min_count=1).reset_index()
    )
    ...
    gen_wtavg = weighted_average(gen, wt_avg_dict, idx_unit)

    idx_plant = ["plant_id_eia", "report_date"]
    gen_agg = pd.merge(
        gen_gpb, gen_wtavg, on=idx_unit, how="outer", validate="1:1"
    ).merge(
        gen[
            idx_plant
            + [
...
def calc_annual_capital_addts_ferc1(steam_df, window=3):
    ...
    steam_df["capex_total_shifted"] = steam_df.groupby(
        [x for x in IDX_STEAM if x != "report_year"]
    )[["capex_total"]].shift()
    ...
    steam_df_w_addts = pd.merge(
        ...
        capex_annual_per_mwh=lambda x: x.capex_annual_addt / x.net_generation_mwh,
        capex_annual_per_mw=lambda x: x.capex_annual_addt / x.capacity_mw,
        capex_annual_per_kw=lambda x: x.capex_annual_addt / x.capacity_mw / 1000,
        capex_annual_per_mwh_rolling=lambda x: x.capex_annual_addt_rolling
        / x.net_generation_mwh,
        capex_annual_per_mw_rolling=lambda x: x.capex_annual_addt_rolling
        / x.capacity_mw,
    )
...
def agg_plants_ferc_by_plant_fuel(steam_df):
    ...
    steam_by_fuel = (
        pd.merge(
            weighted_average(
                steam_df, wa_col_dict=WA_COL_DICT_STEAM, idx_cols=idx_plant_fuel
            ),
            (
        ...
        .merge(  # Is this doing nothing??? can I remove it??
            steam_df.groupby(IDX_PLANT_PUDL, as_index=False)[[]].sum(min_count=1),
            on=IDX_PLANT_PUDL,
            how="outer",
        )
        .merge(
            weighted_average(
                steam_df, wa_col_dict=WA_COL_DICT_STEAM, idx_cols=IDX_PLANT_PUDL
            ),
            on=IDX_PLANT_PUDL,
            how="outer",
            suffixes=("", "_plant"),
...
def merge_eia_ferc_simple(unit, steam_df, unit_id_col):
    ...
    gens_w_ferc1 = (
        ...
        .pipe(pudl.helpers.convert_cols_dtypes, "eia")
        .assign(
            opex_nonfuel_per_mwh=lambda x: x.opex_nonfuel_per_mwh.fillna(
                x.opex_nonfuel_per_mwh_plant
            )
        )
    )
    gens_w_ferc1 = (
        ...
        .assign(
            opex_fixed_per_mwh=lambda x: np.where(
                (x.fuel_type_code_pudl.notnull() & (x.fuel_type_code_pudl == "coal")),
                x.opex_nonfuel_per_mwh * 0.811,
                pd.NA,
            ),
            opex_variable_per_mwh=lambda x: np.where(
                (x.fuel_type_code_pudl.notnull() & (x.fuel_type_code_pudl == "coal")),
                x.opex_nonfuel_per_mwh * 0.188,
                pd.NA,
            ),
...
def count_unique_ids_per_plant_part(steam, gen, id_col="unit_id_pudl"):
    ...
    # unit list
    plant_part_idx = {
        "plant": ["plant_id_eia", "report_date"],
        "plant_gen": ["plant_id_eia", "generator_id", "report_date"],
        "plant_technology": ["plant_id_eia", "technology_description", "report_date"],
        "plant_prime_fuel": ["plant_id_eia", "energy_source_code_1", "report_date"],
        "plant_prime_mover": ["plant_id_eia", "prime_mover_code", "report_date"],
        "plant_ferc_acct": ["plant_id_eia", "ferc_acct_name", "report_date"],
    }
    for part_name, idx_part in plant_part_idx.items():
        ...
        part_unit = pd.merge(part_df, gen[idx_part + [id_col]], on=idx_part)
        part_unit_count = (
            part_unit.groupby(idx_part).nunique(dropna=False).add_suffix("_count")
        )
        part_unit_id = (
            part_unit.groupby(idx_part).agg({id_col: str_squish}).add_suffix("_agg")
        )

        steam_w_count = steam_w_count.set_index(idx_part)
        steam_w_count.loc[
            steam_w_count.plant_part_ferc1 == part_name, f"{id_col}_count"
        ] = part_unit_count
        steam_w_count.loc[
            steam_w_count.plant_part_ferc1 == part_name, f"{id_col}_agg"
        ] = part_unit_id
        steam_w_count = steam_w_count.reset_index()

    steam_w_count.loc[
        steam_w_count.plant_part_ferc1 == "plant_unit", f"{id_col}_count"
    ] = 1
    if not steam_w_count[
        (steam_w_count[f"{id_col}_count"] == 1)
        & steam_w_count[f"{id_col}_agg"].str.contains(";")
    ].empty:
...
def count_unique_steam_records_per_id(steam_w_count, unit_id_col):
    ...
    steam_w_count = pd.merge(
        steam_w_count,
        steam_w_count.assign(idx_unit_count=1)
        .groupby(idx_unit + [f"{unit_id_col}_count"], dropna=False)[["idx_unit_count"]]
        .count()
        .reset_index(),
...
def agg_one_id_steam(steam_w_count, unit_id_col):
    ...
    cols_ferc1 = (
        ...
        + [
            #"opex_nonfuel",
            "net_generation_mwh", "capex_total"]
        + COLS_CAPEX
    )
    # ['opex_nonfuel_per_mwh', 'capex_per_mw']
    steam_1_1_agg = steam_1_1.loc[:, idx_unit + cols_ferc1].assign(steam_agg_type="1:1")
    steam_1_m_wtg = weighted_average(
        steam_1_m.copy(), WA_COL_DICT_STEAM, idx_cols=idx_unit
    )
    steam_1_m_sum = steam_1_m.groupby(idx_unit, as_index=False)[
        [
            #"opex_nonfuel",
            "net_generation_mwh", "capex_total"] + COLS_CAPEX
    ].sum(min_count=1)
    steam_1_m_agg = steam_1_m_wtg.merge(steam_1_m_sum, on=idx_unit, how="outer").assign(
        steam_agg_type="1:m"
    )
...
def merge_eia_ferc_unit(gen, steam_df, unit_id_col="unit_id_pudl"):
    ...
    steam = prep_ferc1_to_eia(ferc1_to_eia, steam_df).pipe(merge_gem_w_df, "ferc")
    eia_ferc_unit = (
        count_unique_ids_per_plant_part(steam, gen, id_col=unit_id_col)
        ...
        .merge(
            ...
            gen.loc[:, ["plant_id_pudl", "plant_id_eia"]].drop_duplicates().dropna(),
            how="left",
            validate="m:1",
        )
    )
...
def merge_eia_ferc(gen, steam_df, unit_id_col, unit=None, debug=False):
    ...
    idx_unit = ["report_date", "plant_id_eia", unit_id_col]
    ferc_merge = pd.merge(
        eia_ferc_fuel.loc[:, list(eia_ferc_unit.columns) + ["fuel_type_code_pudl"]],
        eia_ferc_unit,
    ...
    ferc_merge = ferc_merge.assign(
        ferc1_source_level=lambda x: np.where(
            x.opex_nonfuel_per_mwh_unit.notnull(),
            "unit",
            np.where(x.opex_nonfuel_per_mwh_plant_fuel.notnull(), "plant_fuel", pd.NA),
        ),
    ...
    if not debug:
        ...
        ferc_merge = ferc_merge.drop(
            columns=(
                ferc_merge.filter(like="_unit") + ferc_merge.filter(like="_plant_fuel")
            ).columns
        )
    unit_w_ferc = pd.merge(
        unit,
        ferc_merge,
        on=idx_unit + ["plant_id_pudl"],
        how="left",
        ...
        capex_source=lambda x: np.where(
            ...
            "ferc1",
            np.where(x.capex_annual_per_kw_nems.notnull(), "nems", pd.NA),
        ),
        capex_annual_per_kw=lambda x: x.capex_annual_per_kw.fillna(
            x.capex_annual_per_kw_nems
        ),
    )
...
def label_multi_method_assoc(unit_w_ferc):
    ...
    """
    assoc_types = (
        unit_w_ferc.groupby(IDX_PLANT_PUDL, dropna=False)[["ferc1_source_level"]]
        .nunique()
        .reset_index()
        .assign(
            eia_ferc_merge_multi_method_plant=lambda x: np.where(
                x.ferc1_source_level > 1, True, False
            )
        )
        .drop(columns=["ferc1_source_level"])
    )

    gens_w_ferc1_w_label = unit_w_ferc.merge(assoc_types, on=IDX_PLANT_PUDL)
    return gens_w_ferc1_w_label


def _check_merge_eia_ferc(gens_w_ferc1):
    gens_w_ferc1 = gens_w_ferc1.assign(
        plant_fuel_v_unit_diff=lambda x: x.opex_nonfuel_per_mwh_plant_fuel
...
def get_gem():
    ...
    gem = gem[
        ["plant_name", "unit_name", "other_ids_location", "other_ids_unit", "gem_unit_id"]
    ].add_suffix("_gem")
    ...
    gem = gem.loc[lambda x: x["other_ids_location_gem"] != "not found"]
    gem = (gem.dropna(subset=["other_ids_location_gem", "other_ids_unit_gem"]))
 
    gem.loc[:, "plant_id_eia"] = (
        gem.other_ids_location_gem.apply(lambda x: literal_eval(x)["EIA"])
        .astype("float")
        .astype("Int64")
    )
    ...
    gem = (
        gem.reset_index()
        .rename(columns={"index": "unit_id_gem"})
...
def convert_gem_cols_to_gen_ids(gem):
    ...
    split_ids = gem.generator_ids_eia.str.split(pat=", ", expand=True).add_prefix("id_")
    units_gem = (
...
def merge_gem_w_df(df, df_source="eia"):
    ...
    if (missing_plants.plant_id_eia.nunique() > 27) & (df_source == "eia"):
        raise AssertionError(
            "We have more GEM plants that we didn't find EIA matches for than"
            f" expected. {missing_plants.plant_id_eia.nunique()} instead of 27"
        )
...
def get_nems_headers():
    ...
    headers = list(
        pd.read_excel(
            pathlib.Path.cwd().parent / "inputs" / "pltf860defs_aeo2018.xlsx"
        ).T.loc["Definition"]
    )
    counts = Counter(headers)
    suffix_counter = defaultdict(lambda: itertools.count(1))
    headers_non_dupes = [
        elem if counts[elem] == 1 else elem + f"_{next(suffix_counter[elem])}"
        for elem in headers
    ]
    ...
    nems_df = (
        ...
        .rename(
            columns={
                ...
                "Annual Investment in Capital Additions (87$/kW)": "capex_annual_per_kw_87",
...
def prep_nems():
    ...
    nems_df = (
        get_nems()
        .groupby(by=IDX_GEN + ["report_year"], as_index=False)[
            [
                "capacity_factor",
                "capacity_mw",
                "fixed_om_kw_87",
    ...
    nems_df = nems_df.assign(
        net_generation_mwh_nems=lambda x: x.capacity_factor * 8760 * x.capacity_mw,
        fixed_om=lambda x: x.fixed_om_kw * 1000 * x.capacity_mw,
        fixed_om_mwh=lambda x: x.fixed_om / x.net_generation_mwh_nems,
        variable_om=lambda x: x.variable_om_mwh * x.net_generation_mwh_nems,
        # variable_om_mwh=lambda x: (x.variable_om_mwh),
        fix_var_om_mwh=lambda x: x.variable_om_mwh + x.fixed_om_kw,
        fixed_v_total_ratio=lambda x: x.fixed_om / (x.fixed_om + x.variable_om),
        var_v_total_ratio=lambda x: x.variable_om / (x.fixed_om + x.variable_om),
...
def _agg_nems_to_plant_fuel(nems_df):
    ...
    nems_agg = nems_df.groupby(by=IDX_PLANT_FUEL).agg(
        {
            "variable_om": "sum",
            "fixed_om": "sum",
            "capacity_mw": "sum",
    ...
    nems_cost_df = pd.merge(nems_agg, nems_wtav, on=IDX_PLANT_FUEL, how="outer").assign(
        fixed_v_total_ratio=lambda x: x.fixed_om / (x.fixed_om + x.variable_om),
        var_v_total_ratio=lambda x: x.variable_om / (x.fixed_om + x.variable_om),
        fix_var_om=lambda x: x.fixed_om + x.variable_om,
    )
...
def calc_inflation_nems(nems_df_raw, cols_to_convert, drop=True):
    ...
    nems_df_raw = pd.merge(
        nems_df_raw, nems_idx, on=["report_year"], how="left", validate="m:1"
    )
...
def add_nems(gens_w_ferc1, pudl_out):
    ...
    missing = len(nems_merge_df[nems_merge_df.plant_id_pudl.isnull()]) / len(
        nems_merge_df
    )
...
def get_average_fix_v_var_ratios(gens_w_ferc1_nems):
    ...
    ft_gb = gens_w_ferc1_nems.groupby(["fuel_type_code_pudl", "report_date"])[
        ["fixed_v_total_ratio", "var_v_total_ratio"]
    ]
    ft_fix_var_avg = pd.merge(
        ft_gb.mean().dropna(),
        ft_gb.std().dropna(),
        right_index=True,
        left_index=True,
    ...
    ft_fix_var_avg = (
        ft_fix_var_avg.loc[ft_fix_var_avg.fixed_v_total_ratio_std < 0.1]
        .reset_index()
...
def fill_in_opex_w_nems(gens_w_ferc1_nems):
    ...
    gens_w_ferc1_nems_filled = gens_w_ferc1_nems.merge(
        ft_fix_var_avg,
...
def get_cems(epacems_path, years=()):
    ...
    idx_cols_cems = ["year", "plant_id_eia", "emissions_unit_id_epa"]
    ...
    my_cems_dd = (
        ...
        .astype({"year": int})
        .groupby(idx_cols_cems)[COLS_SUM_CEMS]
        .sum()
    ).reset_index()
    cems_by_boiler = (
        my_cems_dd.compute()
        .rename(columns={"emissions_unit_id_epa": "boiler_id", "year": "report_year"})
        .assign(report_date=lambda x: pd.to_datetime(x.report_year, format="%Y"))
    )
...
def stuff(cems_by_boiler, gen, pudl_out, unit_id_col):
    ...
    eia_with_boiler_id = (
        pudl_out.bga_eia860()[IDX_BOILER + ["unit_id_pudl", "generator_id"]]
        .pipe(merge_gem_w_df, df_source="bga")[IDX_BOILER + [unit_id_col]]
        .drop_duplicates()
    )
    ...
    eia_cems_merge = (
        ...
        .reset_index()
        .assign(
            so2_mass_tons=lambda x: x.so2_mass_lbs / 2000,
        ...
        .merge(
            gen.groupby(idx_unit_cols, dropna=False, as_index=False).agg(
                {"unit_id_eia": str_squish, "generator_id": str_squish}
            ),
            on=idx_unit_cols,
            how="left",
            validate="1:1",
...
def weighted_average(df, wa_col_dict, idx_cols):
    ...
    for data, weight in wa_col_dict.items():
        ...
        df.loc[:, "_weight_where_notnull"] = df.loc[:, weight] * pd.notnull(
            df.loc[:, data]
        )
...
composite_id_assign = composite_unit_gen_id = lambda z: np.where(
    z.unit_id_pudl.notnull() | z.generator_id.notnull(),
    (
        z.unit_id_pudl.astype(pd.StringDtype()).fillna(
            "genid-" + z.generator_id.astype(str)
        )
    ),
    pd.NA,
)
